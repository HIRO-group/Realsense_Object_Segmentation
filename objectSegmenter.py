...
color_sensor.set_option(rs.option.exposure, 700)
...
cv2.createTrackbar('Depth constant', 'controls' , 1, 20000 , empty)
...
cv2.setTrackbarPos('Depth constant', 'controls', depth_constant_default)
	...
	while True:
		...
		if(calibrating):
			...
			if framesCaptured < numFramesToSkip:
				...
				color_difference_threshold_lighter_default = int(np.max([(1000/8333)*euclidean_length + (63324/83333),3]))
				color_difference_threshold_darker_default = int(np.max([(2400/8333)*euclidean_length + (33649/83333),5]))
				...
				if averageDepthInMeters > 2:
					...
					depth_constant = cv2.getTrackbarPos('Depth constant', 'controls')
					depth_difference_threshold = (averageDepthInMM**2)*(1/depth_constant)
		...
		#------------------------------------------------ SEGMENTATION ------------------------------------------------------------------------
			...
			# Get the depth constant, which is used in the depth difference threshold
			depth_constant = cv2.getTrackbarPos('Depth constant', 'controls')
			...
			depth_colormap = cv2.morphologyEx(depth_colormap, cv2.MORPH_OPEN, kernel1)
			depth_colormap= cv2.morphologyEx(depth_colormap, cv2.MORPH_CLOSE, kernel1)

			# Render the depth subtraction window
			if ignoring_depth:
				cv2.imshow(depthWindow,ignoring_image)
			else:
				cv2.imshow(depthWindow,depth_colormap)
		...
		#------------------------------------UNION OF DEPTH AND RGB SUBTRACTION--------------------------------------------------------
			...
			combined = np.where(culled_color_image != 0, white_color,black_color)
			combined = np.where(binary_depth_image != 0,white_color,combined)
			...
			#combined_colormap = cv2.morphologyEx(combined_colormap, cv2.MORPH_OPEN, kernel1)
			#combined_colormap = cv2.morphologyEx(combined_colormap, cv2.MORPH_CLOSE, kernel1)
